<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">

    <meta name="Description" CONTENT="Meccfor è un’azienda esperta, dinamica e professionale che opera nel settore della fornitura di componenti e prodotti finiti oltre che nella costruzione di parti a disegno.">

    <meta name=”robots” content="index, nofollow">
	<meta name="viewport" content="width=device-width, initial-scale=1">

	<?php include('inc/meta.php'); ?>

	<?php include('inc/favicons.php'); ?>

	<title>MECCFOR - Design & Forniture Componenti Meccanici</title>
	<style type="text/css"><?php include('assets/css/mc.css'); ?></style>
</head>
<body>

	<div class="mc-container">
		<header class="mc-header">

			<nav role="navigation">
				<div id="menuToggle">
					<input type="checkbox" />
					<span></span>
					<span></span>
					<span></span>

					<?php include('inc/menu.php'); ?>
				</div>
			</nav>
			<div class="mc-heading">
				<h1><img src="assets/graphics/logo-full.svg" width="313" height="88" alt="MECCFOR - Design & Forniture Componenti Meccanici" title="MECCFOR - Design & Forniture Componenti Meccanici" class="mc-logo-full"></h1>
			</div>

		</header>


		<div class="mc-content mc-content-homepage">
			<img width="1920" height="1280" alt="MECCFOR - Design & Forniture Componenti Meccanici" srcset="
			./assets/graphics/meccfor-cover-home-low.jpg 960w,
			./assets/graphics/meccfor-cover-home.jpg 1920w
			"
			sizes="100vw" class="mc-cover-image">

			<div class="mc-bigsymbol">
				<h2 class="mc-tagline">Design & Forniture Componenti Meccanici</h2>
			</div>

			<footer>&copy; Meccfor srl - All right reserved - VAT n. 02588850343</footer>

		</div>

	</div>

	<script src="assets/js/mc.js"></script>
</body>
</html>